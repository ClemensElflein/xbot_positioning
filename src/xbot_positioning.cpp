...
void onWheelTicks(const xbot_msgs::WheelTick::ConstPtr &msg) {
    ...
    uint32_t dl=msg->wheel_ticks_rl - last_ticks.wheel_ticks_rl;
    uint32_t dr=msg->wheel_ticks_rr - last_ticks.wheel_ticks_rr;
    ...
    double d_wheel_l = (double) dl * (1/(double)msg->wheel_tick_factor);
    double d_wheel_r = (double) dr * (1/(double)msg->wheel_tick_factor);
